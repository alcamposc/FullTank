...
public class TestSeccionGruposInteresWeb {
  ...
  public void testCU10() throws Exception {
    ...
    Assert.assertEquals("GRUPOS DE INTERES", driver.findElement(By.xpath("//div[@id='Grupos_de_interes']/div/h2")).getText());
    try {
      Assert.assertEquals("Esta aplicacion está destinada principalmente a todas aquellas personas que dispongan de un vehículo o puedan conducir uno, debido a que se pretende ayudar al usuario a escoger la gasolinera que más se ajuste a sus necesidades.", driver.findElement(By.xpath("//div[@id='Grupos_de_interes']/div/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      Assert.assertEquals("Otro grupo de interés son aquellas personas sin carnet que conduzcan automoviles o los copilotos de los vehículos para poder informar al piloto.", driver.findElement(By.xpath("//div[@id='Grupos_de_interes']/div/p[2]")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      Assert.assertEquals("Todos los grupos de interés que utilizan está aplicación se encuentran atraidos por el mismo objetivo, que es el de poder ahorrar dinero a la hora de repostar combustibles en la gasolinera más cercana posible a su ubicación actual.", driver.findElement(By.xpath("//div[@id='Grupos_de_interes']/div/p[3]")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
